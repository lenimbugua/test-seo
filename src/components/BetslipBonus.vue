...
<template>
  <div class="flex justify-between items-center px-4">
    <div class="flex items-center space-x-1">
      <!-- <GiftIcon class="w-4 h-4 text-fuchsia-500" /> -->
      <svg
      ...
      <span class="text-fuchsia-500 text-xs">Boost Bonus</span>
    </div>
    <div class="flex justify-end items-center">
      <div class="flex items-center space-x-1">
        <span class="dark:text-slate-50 text-sm font-black">
          {{ calculateBoostBonus(calculatePossibleWin(betslip, stake)) }}
        </span>
        <span class="text-xs dark:text-slate-400">KES</span>
      </div>
    </div>
  </div>
</template>
