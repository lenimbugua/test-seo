...
<template>
  ...
  <button
    ...
    :class="[
      ...
      outcomeIsSelected
        ? 'bg-[#0F8F21]/80'
        : 'bg-gray-300/70  dark:bg-[#2E283E]',
    ]"
    class="flex-auto rounded-md min-w-20 px-4 py-3 flex flex-col text-center font-medium cursor-pointer transition relative"
    @click="handleSelect($event)"
  >
    <div v-if="outcomeIsLocked()">
      <ThePadlock />
    </div>
    <div
      v-else
